...
class PessoaForm(forms.ModelForm):
    ...
    def clean_cpf(self) -> str:
        ...
        exists = (
            Treinador.objects.filter(cpf=cpf_normalizado).exists() or
            Aluno.objects.filter(cpf=cpf_normalizado).exists()
        )
        if exists:
    ...
    def clean_num_telefone(self) -> str:
        ...
        exists = (
            Treinador.objects.filter(num_telefone=telefone_normalizado).exists() or
            Aluno.objects.filter(num_telefone=telefone_normalizado).exists()
        )
        if exists:
