...
class Pessoa(models.Model):
    ...
    genero = models.CharField(
        max_length=12, choices=[("masculino", "Masculino"), ("feminino", "Feminino")]
    )
...
class Aluno(Pessoa):
    ...
    treinador = models.ForeignKey(
        Treinador, on_delete=models.CASCADE, related_name="alunos"
    )
    objects = ActiveManager()
...
class EscalaPoms(models.Model):
    ...
    freq_cardiaca_media = models.IntegerField(
        null=True, blank=True, help_text="Frequência cardíaca média (BPM)"
    )
...
class ClassificacaoRecomendacao(models.Model):
    ...
    escala = models.OneToOneField(
        EscalaPoms, on_delete=models.CASCADE, related_name="classificacao"
    )
