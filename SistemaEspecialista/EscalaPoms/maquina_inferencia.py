...
def carregar_regras_classificacao_poms(
    caminho_arquivo: str,
    ...
    for bloco in padrao_bloco.finditer(texto_regras):
        ...
        var_saida, rotulo_nivel = [
            parte.strip() for parte in texto_conclusao.split("=")
        ]

        # Captura comparações, ex: ('soma_tensao','>','5')
        comparacoes = re.findall(
            r"(\w+)\s*(<=|>=|<|>)\s*(\d+(?:\.\d+)?)", texto_condicao
        )
        if not comparacoes:
            continue
...
def carregar_sugestoes_treino_poms(caminho_arquivo: str) -> Dict[str, Dict[str, str]]:
    ...
    for bloco in padrao_bloco.finditer(texto_regras):
        ...
        mapa_sugestoes.setdefault(nome_variavel_nivel, {})[
            rotulo_nivel
        ] = texto_sugestao
...
def classificar_e_recomendar_poms(
    caminho_arquivo_regras: str, somas_emocoes: Dict[str, float]
) -> Dict[str, str]:
    """
    ...
    intervalos_classificacao = carregar_regras_classificacao_poms(
        caminho_arquivo_regras
    )
